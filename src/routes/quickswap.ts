...
import Quickswap from '../services/quickswap';
...
const quickswap = new Quickswap(globalConfig.getConfig('ETHEREUM_CHAIN'));
quickswap.generate_tokens();
setTimeout(quickswap.update_pairs.bind(quickswap), 2000);
const fees = new Fees();
...
const getErrorMessage = (err: string) => {
  ...
  */
  ...
  if (err.includes('failed to meet quorum')) {
    message = 'Failed to meet quorum in quickswap';
...
router.post('/', async (_req: Request, res: Response) => {
  ...
  */
  res.status(200).json({
    network: quickswap.network,
    provider: quickswap.provider.connection.url,
    quickswap_router: quickswap.router,
    connection: true,
    timestamp: Date.now(),
  });
});
...
router.get('/start', async (req: Request, res: Response) => {
  /*
    GET: /eth/quickswap/start?pairs=["WETH-USDC"]&gasPrice=30
  */
...
router.post('/trade', async (req: Request, res: Response) => {
  ...
  */
  ...
  const wallet = new ethers.Wallet(privateKey, quickswap.provider);
  ...
  if (baseTokenContractInfo && quoteTokenContractInfo) {
    ...
    try {
      // fetch the optimal pool mix from quickswap
      const result: any =
        side === 'BUY'
          ? await quickswap.priceSwapOut(
              quoteTokenAddress, // tokenIn is quote asset
              baseTokenAddress, // tokenOut is base asset
              amount
            )
          : await quickswap.priceSwapIn(
      ...
      if (result && result.trade && result.expectedAmount) {
        ...
        if (side === 'BUY') {
          ...
          if (!limitPrice || price <= limitPrice) {
            ...
            const tx = await quickswap.swapExactOut(
              wallet,
              trade,
              baseTokenAddress,
              gasPrice
            ...
            logger.info(
              `quickswap.route - Swap price ${price} exceeds limitPrice ${limitPrice}`
            );
          }
        } else {
          // sell
          ...
          if (!limitPrice || price >= limitPrice) {
            // pass swaps to exchange-proxy to complete trade
            const tx = await quickswap.swapExactIn(
              wallet,
              trade,
              baseTokenAddress,
              gasPrice
            ...
            logger.info(
              `quickswap.route - Swap price ${price} lower than limitPrice ${limitPrice}`
            );
          }
        }
      } else {
  ...
  if (baseTokenContractInfo && quoteTokenContractInfo) {
    ...
    try {
      ...
      if (Object.keys(err).includes('isInsufficientReservesError')) {
        errCode = 200;
        reason =
          statusMessages.insufficient_reserves + ' in ' + side + ' at quickswap';
